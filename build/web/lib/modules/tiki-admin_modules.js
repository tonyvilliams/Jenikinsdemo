...
// $Id: tiki-admin_modules.js 46202 2013-06-06 15:38:30Z jonnybradley $
...
$(".module, #assigned_modules tr").dblclick(function () {showModuleEditForm(this);});
...
var showModuleEditForm = function(item, options) {
	...
	if (item) {
		if ($(item).is("tr")) {		// assigned_modules row dblclicked
			...
			modId = $("a:last", item).attr("href").match(/\d+$/);
			if (modId) {
				modId = modId[0];
				modOrd = $("td:eq(2)", item).text();
				modPos = $(item).parents("table:first").attr("id").match(/_(\w)$/);
				if (modPos) {
					modPos = modPos[1];
				}
			}
